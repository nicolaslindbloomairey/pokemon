...
class Validator {
	...
	baseValidateTeam(team, removeNicknames) {
		...
		if (!team || !Array.isArray(team)) {
			...
			return [`You sent invalid team data. (arrray)If you're not using a custom client, please report this as a bug.`];
		}
		...
		for (let i = 0; i < team.length; i++) {
			if (!team[i]) return [`You sent invalid team data.too many pokes If you're not using a custom client, please report this as a bug.`];
